 ...
 */
export const LUMO_TRIP_SUMMARY_KEY = "_lumo_trip_summary";
// ──────────────────────────────────────────────────────────────────────────
 ...
 */
export function hashTripSummary(payload) {
    const canonical = {
 ...
 */
export function attachTripSummary(body, input) {
    assertValidTripPayload(input.payload);
    if (body === null || typeof body !== "object" || Array.isArray(body)) {
        throw new TypeError("attachTripSummary: `body` must be a plain object (got " +
            (Array.isArray(body) ? "array" : typeof body) +
            ")");
    }
    if (Object.prototype.hasOwnProperty.call(body, LUMO_TRIP_SUMMARY_KEY)) {
        throw new Error(`attachTripSummary: body already carries a \`${LUMO_TRIP_SUMMARY_KEY}\` field; refusing to overwrite.`);
    }
    ...
    return { ...body, [LUMO_TRIP_SUMMARY_KEY]: envelope };
}
/**
 ...
 */
export function extractTripSummary(input) {
    if (input === null || typeof input !== "object" || Array.isArray(input)) {
        return null;
    }
    const env = input[LUMO_TRIP_SUMMARY_KEY];
    if (env === null || typeof env !== "object" || Array.isArray(env))
        return null;
    const rec = env;
    if (rec.kind !== "structured-trip")
        return null;
...
export function stripTripSummary(body) {
    if (body === null || typeof body !== "object")
        return body;
    if (!Object.prototype.hasOwnProperty.call(body, LUMO_TRIP_SUMMARY_KEY))
        return body;
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { [LUMO_TRIP_SUMMARY_KEY]: _removed, ...rest } = body;
    return rest;
}
// ──────────────────────────────────────────────────────────────────────────
// Validation — strict, throws, used by attach and extract
// ──────────────────────────────────────────────────────────────────────────
function assertValidTripPayload(p) {
    ...
    if (typeof p.total_amount !== "string" || !/^\d+(\.\d+)?$/.test(p.total_amount)) {
        throw new TypeError("TripSummaryPayload.total_amount: decimal string required (e.g. \"1247.00\")");
    }
    ...
    for (const leg of p.legs) {
        ...
        for (const d of leg.depends_on) {
            if (!Number.isInteger(d) || d < 1 || d >= leg.order) {
                throw new Error(`TripLegRef[${leg.order}].depends_on contains invalid order ${d}: must be >=1 and < ${leg.order}`);
            }
        }
        // The leg must carry a v0.1 AttachedSummary — minimal shape check.
        if (!leg.summary ||
            typeof leg.summary !== "object" ||
            typeof leg.summary.hash !== "string" ||
            !/^[0-9a-f]{64}$/.test(leg.summary.hash)) {
            throw new TypeError(`TripLegRef[${leg.order}].summary: invalid AttachedSummary (missing or malformed hash)`);
        }
    }
    // Dense 1-indexed ordering: if we have N legs, orders must be {1..N}.
    for (let i = 1; i <= p.legs.length; i += 1) {
        if (!orders.has(i)) {
            throw new Error(`TripSummaryPayload.legs: ordering must be dense 1..${p.legs.length}, missing ${i}`);
        }
    }
}
